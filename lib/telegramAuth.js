...
function verifyTelegramInitData(initData, botToken) {
  ...
  // Сравнение через timingSafeEqual, а не === : обычное строковое
  // сравнение прерывается на первом несовпадающем символе, и в теории
  // (при очень большом числе точных измерений задержки ответа) это
  // может слить по крупицам подпись атакующему. timingSafeEqual всегда
  // сравнивает за одинаковое время независимо от того, где нашлось
  // расхождение. SHA-256 в hex — всегда 64 символа; если длина не та
  // (обрезанный или битый hash) — подпись в любом случае неверна.
  const hashIsValidFormat = /^[0-9a-f]{64}$/.test(hash);
  const signaturesMatch = hashIsValidFormat &&
