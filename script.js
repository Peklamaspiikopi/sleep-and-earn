document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (directAdBtn) {
        directAdBtn.addEventListener('click', async () => {
            ...
            if (videoController) {
                ...
                alert(currentLang === 'ru' ? 'Реклама сейчас недоступна, попробуй чуть позже' : 'Ads are unavailable right now, try again later');
                onFail();
            }
        });
    }
    ...
    function updateTopicsRemainingLine(lockedTopicsRemaining, secretRemaining) {
        ...
        topicsRemainingLine.innerText = currentLang === 'ru'
            ? `Осталось открыть тем: ${l} · секретных дилемм: ${s}`
            : `Topics left to unlock: ${l} · secret dilemmas left: ${s}`;
    }
        ...
        topicSelectorGrid.querySelectorAll('.topic-tile[data-topic]').forEach((btn) => {
            btn.addEventListener('click', () => {
                if (btn.dataset.unlocked !== 'true') {
                    alert(currentLang === 'ru'
                        ? 'Тема пока закрыта. Открой её кнопкой «Открыть новую тему» выше (по одному ключу за раз, по порядку).'
                        : 'This topic is still locked. Unlock it with the "Unlock new topic" button above (one key at a time, in order).');
                    return;
                }
        ...
        dilemmaCheckpointBtn.addEventListener('click', async () => {
            ...
            const finish = async () => {
                try {
                    ...
                    userState.game_tokens = result.gameTokens;
                    updateGameJetonsDisplay();
                    alert(currentLang === 'ru' ? `🎉 +${result.reward} жетонов!` : `🎉 +${result.reward} tokens!`);
                    loadDilemma(currentDilemmaTopic);
                } catch (e) {
                    alert(e.message);
                } finally {
