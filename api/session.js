...
const {
  ...
  DAILY_GAME_LADDER_FULL, DAILY_GAME_LADDER_SKIP, DAY7_POSITION, DAY7_BONUS_COINS,
  TERMINAL_CLOSED, TERMINAL_CLOSED_MESSAGE, CHECKPOINT_TOKEN_REWARD,
...
async function handleBannerStart(req, res, telegramId) {
  ...
  if (error || !session) {
    { console.error('sessions insert failed (banner):', error); return res.status(500).json({ error: 'Не удалось начать баннер' }); }
  }
...
async function handleFortuneStart(req, res, telegramId) {
  ...
  if (error || !session) {
    console.error('sessions insert failed (fortune):', error);
...
async function handleCheckpointStart(req, res, telegramId) {
  ...
  if (error || !session) {
    ...
    console.error('sessions insert failed (checkpoint):', error);
    return res.status(500).json({ error: 'Не удалось начать' });
  }
...
async function handleCheckpointComplete(req, res, telegramId) {
  ...
  for (let attempt = 0; attempt < 5 && !committed; attempt++) {
    ...
    const { data: result } = await supabaseAdmin
      .from('users')
      .update({ game_tokens: attemptTokens })
      .eq('telegram_id', telegramId)
      .eq('game_tokens', base)
      .select()
      .maybeSingle();
...
async function handleDirectAdStart(req, res, telegramId) {
  ...
  if (error || !session) { console.error('sessions insert failed:', error); return res.status(500).json({ error: 'Не удалось начать' }); }
  return res.status(200).json({ sessionId: session.id });
}
...
async function handleGameStart(req, res, telegramId) {
  ...
  if (error || !session) { console.error('sessions insert failed (game round):', error); return res.status(500).json({ error: 'Не удалось начать раунд' }); }
...
async function handleStreakAdComplete(req, res, telegramId) {
  ...
  const newTokens = (user.game_tokens || 0) + reward;
  const bonusCoins = isBigDay ? DAY7_BONUS_COINS : 0;
  const newBalance = (user.balance || 0) + bonusCoins;
  ...
  if (bonusCoins > 0) await logTransaction(supabaseAdmin, telegramId, 'streak_day7_bonus', bonusCoins, updated.balance);
  await creditReferralIfEligible(telegramId, newStreakCount, user.referred_by, user.referral_credited);

  return res.status(200).json({
    ok: true, reward, gameTokens: updated.game_tokens, streakCount: newStreakCount,
    dayPosition: pos + 1, bonusCoins, balance: updated.balance,
  });
}
